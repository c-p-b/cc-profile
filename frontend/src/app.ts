...
export class ClaudeApp extends LitElement {
  ...
  private processData() {
    ...
    const conversationModels = new Set(
      this.conversations.flatMap((c) => Array.from(c.models)),
    );
    const processedPairModels = new Set(
      this.processedPairs.map((p) => p.model),
    );
    const rawPairModels = new Set(
      this.data.rawPairs.map((pair) => pair.request.body?.model || "unknown"),
    );
  ...
  private switchView(view: "conversations" | "api-summary" | "raw" | "json") {
  ...
  private get filteredConversations() {
    return this.conversations.filter((c) => {
      ...
      return Array.from(c.models).some((model) =>
        this.selectedModels.has(model),
      );
    });
  }

  private get filteredProcessedPairs() {
    return this.processedPairs.filter((pair) =>
        ...
        <div class="max-w-[60em] mx-auto p-4">
          <div class="mb-8">
            <div class="mb-4 text-center">
              ...
              <span class="text-vs-muted ml-8"
                >${this.data.timestamp || new Date().toISOString()}</span
              >
            </div>
            ...
            <div class="mb-8 text-center">
              <span
                ...
                class="cursor-pointer mr-12 ${this.currentView ===
                "conversations"
                  ? "text-vs-nav-active"
              ...
              <span
                @click=${() => this.switchView("api-summary")}
                class="cursor-pointer mr-12 ${this.currentView === "api-summary"
                  ? "text-vs-nav-active"
                  : "text-vs-text hover:text-vs-accent"}"
              >
                api calls (${this.filteredProcessedPairs.length})
              </span>
              <span
            ...
            ${modelCounts.size > 1 && this.currentView !== "raw"
              ? html`
                  <div class="mb-4 text-center">
                    ${Array.from(modelCounts.entries()).map(
                      ([model, _count]) => {
                        return html`
                          <span
                          ...
                          >
                            ${this.selectedModels.has(model) ? "[x]" : "[ ]"}
                            ${model}
                          </span>
                        `;
                      },
          ...
          <div>
            ${this.currentView === "conversations"
              ? html`
                ...
                `
              : ""}
            ${this.currentView === "api-summary"
              ? html`
                  <div id="api-summary-view">
                    ${this.filteredProcessedPairs.length === 0
                      ? html`<div>No API calls found for selected models.</div>`
                      : html`<api-summary-view
                          .processedPairs=${this.filteredProcessedPairs}
                        ></api-summary-view>`}
                  </div>
                `
              : ""}
