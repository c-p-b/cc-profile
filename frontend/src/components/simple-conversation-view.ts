        ...
        ${content.map((block) => {
          if (block.type === "text") {
            ...
            if (block.name === "Write") {
              ...
              return html`
                <div class="mt-4 mb-4">
                  <div class="text-vs-type px-4 break-all">
...
${JSON.stringify(block, null, 2)}</pre
  ...
  private formatStringContent(content: string): TemplateResult {
    ...
    const systemReminderRegexEscaped =
      /&lt;system-reminder&gt;([\s\S]*?)&lt;\/system-reminder&gt;/g;
    const systemReminderRegexRaw =
      /<system-reminder>([\s\S]*?)<\/system-reminder>/g;
    ...
    return html`
      ${mainContent
        ? html`<div class="mt-4 markdown-content">
      ...
      ${systemReminders.length > 0
        ? this.renderCollapsibleSection(
                ...
                (reminder, index) => html`
                  <div class="mb-4">
                    ${systemReminders.length > 1
                      ? html`<div class="text-vs-function font-bold mb-2">
                          Reminder ${index + 1}:
                        </div>`
                      : ""}
            ...
            </div>`,
            {
              ...
              count:
                systemReminders.length > 1 ? systemReminders.length : undefined,
            },
          )
        : ""}
    `;
    ...
    if (response.content && Array.isArray(response.content)) {
      return html`
        ${response.content.map((block) => {
          if (block.type === "text") {
            return html`<div class="mt-4 markdown-content">
...
${JSON.stringify(block, null, 2)}</pre
  ...
  private formatSingleParam(
    ...
    return paramValue
      ? html`${toolName}(<span class="text-vs-text"
            >${this.unescapeHtml(paramValue)}</span
          >)`
      : html`${toolName}`;
  }
  ...
  private wrapInScrollable(
    content: TemplateResult | string,
    usePreFormatting: boolean = true,
  ): TemplateResult {
    if (usePreFormatting && typeof content === "string") {
      return html`
        <div class="overflow-x-auto">
          <pre
            class="text-vs-text m-0"
...
${content}</pre
  ...
  private renderCollapsibleSection(
    ...
    return html`
      <div class="${containerClasses}">
        <div
          class="cursor-pointer hover:text-white transition-colors ${titleClasses}"
          @click=${this.toggleContent}
        >
  ...
  private getToolDisplayName(toolUse: any, _toolResult?: any): TemplateResult {
    ...
    switch (toolName) {
      ...
      case "LS":
        if (input?.path) {
          ...
          if (input?.ignore) {
            const ignoreStr = input.ignore
              .map((p: string) => this.unescapeHtml(p))
              .join(", ");
      ...
      case "Edit":
        return input?.file_path
          ? this.formatSingleParam(
              toolName,
              this.unescapeHtml(input.file_path).split("/").pop(),
            )
          : html`${toolName}`;
      case "MultiEdit":
        if (input?.file_path) {
          ...
          return this.formatMultiParam(toolName, [
            fileName,
            `${editCount} edits`,
          ]);
        }
      ...
      case "NotebookRead":
        return input?.notebook_path
          ? this.formatSingleParam(
              toolName,
              this.unescapeHtml(input.notebook_path).split("/").pop(),
            )
          : html`${toolName}`;
      case "NotebookEdit":
        if (input?.notebook_path && input?.cell_number !== undefined) {
          const fileName = this.unescapeHtml(input.notebook_path)
            .split("/")
            .pop();
      ...
      case "WebFetch":
        return input?.url
          ? this.formatSingleParam(toolName, input.url)
          : html`${toolName}`;
      case "WebSearch":
        return input?.query
    ...
    if (toolName === "TodoWrite" && input?.todos) {
      ...
      return this.wrapInScrollable(
        html`${todos.map((todo: any) => {
          ...
          return html`
            <div
              class="overflow-hidden whitespace-nowrap text-ellipsis ${statusClass}"
            >
              • ${todo.content}
            </div>
          `;
  ...
  private renderToolResult(_toolResult: any, toolUse?: any): TemplateResult {
    return html`
      <div class="mb-4">
        <div
        ...
        <div
          class="bg-vs-bg-secondary mx-4 p-4 text-vs-text hidden overflow-x-auto"
        >
          <pre
...
${typeof toolResult.content === "string"
  ...
  private renderToolContainer(
    ...
    const contentDiv =
      customContent || html`${this.renderToolUseContent(toolUse)}`;
    ...
    return html`
      <div class="mt-4 mb-4">
        <div
          class="${headerClasses}"
          @click=${isClickToExpand ? this.toggleContent : undefined}
        >
          ${isCollapsible
            ? html`<span class="mr-2">[${isCollapsed ? "+" : "-"}]</span>`
            : ""}
  ...
  private renderDiff(oldStr: string, newStr: string): TemplateResult[] {
    ...
    for (const part of diff) {
      ...
      for (const line of lines) {
        if (part.added) {
          diffLines.push(
            html`<div class="bg-green-600/20">
              <pre class="text-vs-text m-0">+ ${line}</pre>
            </div>`,
          );
        } else if (part.removed) {
          diffLines.push(
            html`<div class="bg-red-600/20">
              <pre class="text-vs-text m-0">- ${line}</pre>
            </div>`,
          );
        } else {
          diffLines.push(
            html`<div><pre class="text-vs-text m-0">  ${line}</pre></div>`,
          );
        }
      }
    }
  ...
  private hasTools(conversation: SimpleConversation): boolean {
    return !!(
      conversation.finalPair.request.tools &&
      conversation.finalPair.request.tools.length > 0
    );
  }
    ...
    return html`
      ${tools.map((tool) => {
        if ("name" in tool && tool.name) {
          const description =
            ("description" in tool && tool.description) || "No description";
                      ...
                      return html`
                        ...
                        ${Object.entries(schema.properties).map(
                          ([paramName, paramDef]) => {
                            ...
                            const required = schema.required?.includes(
                              paramName,
                            )
                              ? " (required)"
                              : "";
  ...
  private renderConversationContent(
                    ...
                    <div class="hidden mt-8">
                      ...
                      <div class="border border-red-700 p-4 mb-0">
                        ...
                        <div class="text-vs-muted">
                          ...
                          <span class="ml-4"
                            >${conversation.messages.length + 1} messages</span
                          >
                        </div>
                      </div>
