...
// TODO: Update to use OTLP format
// import { HTMLGenerator } from "./html-generator";
import { RawPair } from "./types.js";
import {
  SharedConversationProcessor,
  SimpleConversation,
  ProcessedPair,
} from "./shared-conversation-processor.js";
...
export class IndexGenerator {
  private traceDir: string = ".cc-profile";
  ...
  private findLogFiles(): string[] {
    ...
    return files
      .filter((file) =>
        file.match(/^log-\d{4}-\d{2}-\d{2}-\d{2}-\d{2}-\d{2}\.jsonl$/),
      )
  ...
  private async processLogFile(logFile: string): Promise<LogSummary | null> {
    ...
    const timestamp = logFile.match(
      /log-(\d{4}-\d{2}-\d{2}-\d{2}-\d{2}-\d{2})\.jsonl$/,
    )?.[1];
    ...
    if (needsRegeneration) {
      console.log(
        `  Generating summary (${needsRegeneration ? "missing or outdated" : "up to date"})...`,
      );

      // TODO: Generate HTML from OTLP format
      // Ensure HTML file exists
      if (!fs.existsSync(htmlPath)) {
        console.log(`  Skipping HTML generation - needs OTLP update`);
        return null;
      }
      ...
      const nonCompactedConversations = conversations.filter(
        (conv) => !conv.compacted && conv.messages.length > 2,
      );
      console.log(
        `  Found ${nonCompactedConversations.length} non-compacted conversations (>2 messages)`,
      );
  ...
  private async extractConversations(
    ...
    const processedPairs: ProcessedPair[] =
      this.conversationProcessor.processRawPairs(rawPairs);
...
SUMMARY: [summary]`;
      ...
      if (!titleMatch || !summaryMatch) {
        console.log(
          `    Failed to parse Claude response for conversation ${conversation.id}`,
        );
        return null;
      }
      ...
      console.log(
        `    Failed to summarize conversation ${conversation.id}: ${error}`,
      );
      return null;
    }
  }
