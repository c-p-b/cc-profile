...
import type { RawPair } from "./types.js";
...
export class SharedConversationProcessor {
  ...
  processRawPairs(rawPairs: RawPair[]): ProcessedPair[] {
    ...
    for (const pair of rawPairs) {
      ...
      processedPairs.push({
        ...
        timestamp: new Date(
          (pair.request.timestamp || Date.now()) * 1000,
        ).toISOString(),
  ...
  private extractModel(pair: RawPair): string {
    ...
    if (
      pair.request?.body &&
      typeof pair.request.body === "object" &&
      "model" in pair.request.body
    ) {
    ...
    if (
      pair.response?.body &&
      typeof pair.response.body === "object" &&
      "model" in pair.response.body
    ) {
  ...
  mergeConversations(
    ...
    for (const [, systemPairs] of pairsBySystem) {
      ...
      for (const pair of sortedPairs) {
        ...
        const normalizedFirstMessage =
          this.normalizeMessageForGrouping(firstUserMessage);
      ...
      for (const [conversationKey, threadPairs] of conversationThreads) {
        ...
        const finalPair = sortedThreadPairs.reduce((longest, current) => {
          ...
          return currentMessages.length > longestMessages.length
            ? current
            : longest;
        });

        const modelsUsed = new Set(sortedThreadPairs.map((pair) => pair.model));
        const enhancedMessages = this.processToolResults(
          finalPair.request.messages || [],
        );
    ...
    const mergedConversations =
      this.detectAndMergeCompactConversations(allConversations);
  ...
  private processToolResults(messages: MessageParam[]): EnhancedMessageParam[] {
    ...
    for (let i = 0; i < messages.length; i++) {
      ...
      const enhancedMessage: EnhancedMessageParam = {
        ...message,
        toolResults: {},
        hide: false,
      };
          ...
          if (block.type === "tool_use" && "id" in block) {
            ...
            if (pendingToolUses[toolUseId]) {
              ...
              if (!enhancedMessages[messageIndex]) {
                enhancedMessages[messageIndex] = {
                  ...messages[messageIndex],
                  toolResults: {},
                  hide: false,
                };
              }
              enhancedMessages[messageIndex].toolResults![toolUseId] =
                toolResult;
  ...
  private detectAndMergeCompactConversations(
    ...
    for (let i = 0; i < sortedConversations.length; i++) {
      ...
      if (
        ...
        for (let j = 0; j < sortedConversations.length; j++) {
          ...
          if (otherConv.messages.length === currentConv.messages.length - 2) {
            ...
            for (let k = 1; k < otherConv.messages.length; k++) {
              if (
                !this.messagesRoughlyEqual(
                  otherConv.messages[k],
                  currentConv.messages[k],
                )
              ) {
                messagesMatch = false;
                break;
        ...
        if (originalConv) {
          const mergedConv = this.mergeCompactConversation(
            originalConv,
            currentConv,
          );
  ...
  private mergeCompactConversation(
    ...
    return {
      ...
      metadata: {
        ...
        inputTokens:
          (originalConv.metadata.inputTokens || 0) +
          (compactConv.metadata.inputTokens || 0),
        outputTokens:
          (originalConv.metadata.outputTokens || 0) +
          (compactConv.metadata.outputTokens || 0),
        totalTokens:
          (originalConv.metadata.totalTokens || 0) +
          (compactConv.metadata.totalTokens || 0),
      },
    };
  }
    ...
    if (Array.isArray(message.content)) {
      normalizedContent = message.content.map((block) => {
        if (block.type === "text" && "text" in block) {
          let text = block.text;
          text = text.replace(
            /Generated \d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}/g,
            "Generated [TIMESTAMP]",
          );
          text = text.replace(
            /The user opened the file [^\s]+ in the IDE\./g,
            "The user opened file in IDE.",
          );
          text = text.replace(
